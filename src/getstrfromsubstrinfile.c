 ...
 * uasge:
    const char *fullPathToConf = "/home/diogenes/.config/diowapplauncher/icons.cache";
...
char *find_substring_in_file(const char *fullPathToConf, const char *substring) {
	...
	char noIconBuff[strlen(HOME) + 64];
	snprintf(noIconBuff, sizeof(noIconBuff), "%s/.config/diowapplauncher/noicon.svg", HOME);
